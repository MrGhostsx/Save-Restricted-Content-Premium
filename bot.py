...
from config import API_ID, API_HASH, BOT_TOKEN, OWNER_ID, MONGO_URL, REQUIRED_CHANNEL, REQUIRED_GROUP
...
async def check_user_in_channel(user_id):
    try:
        ...
        channel_member = await bot.get_chat_member(REQUIRED_CHANNEL, user_id)
        if channel_member.status in ["left", "kicked"]:
            return False
        
        # Check if user is in group (if required)
        if REQUIRED_GROUP:
            group_member = await bot.get_chat_member(REQUIRED_GROUP, user_id)
...
async def redeem_code(client, message):
    ...
    has_joined = await check_user_in_channel(message.from_user.id)
    if not has_joined:
        join_message = (
            "⚠️ **You must join our channel/group first to use this bot!**\n\n"
            f"Please join: {REQUIRED_CHANNEL}\n"
        )
        if REQUIRED_GROUP:
            join_message += f"And also join our group: {REQUIRED_GROUP}\n"
        join_message += "\nAfter joining, try again."
        await message.reply(join_message, disable_web_page_preview=True)
        return
...
@bot.on_message(filters.private & filters.new_chat_members)
async def welcome_new_user(client, message):
    welcome_msg = (
        "👋 **Welcome to Save Restricted Content 🤖 Bot!**\n\n"
        "To use this bot, you need to:\n"
        f"1. Join our channel: {REQUIRED_CHANNEL}\n"
    )
    if REQUIRED_GROUP:
        welcome_msg += f"2. Join our group: {REQUIRED_GROUP}\n"
    welcome_msg += "\nAfter joining, you can start using the bot commands."
...
async def check_user_approval(client, message):
    ...
    # Allow help and start commands without joining
    if message.command and message.command[0] in ["start", "help", "planinfo", "terms"]:
        await message.continue_propagation()
